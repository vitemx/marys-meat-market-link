    ...
    <nav className="fixed top-0 left-0 right-0 z-40 bg-foreground/90 backdrop-blur-md border-b border-border/20">
      <div className="container flex items-center justify-between h-16 px-4">
        ...
        <div className="hidden md:flex items-center gap-8">
          {links.map((l) => (
            <a key={l.href} href={l.href} className="text-primary-foreground/80 hover:text-primary transition-colors font-body text-sm uppercase tracking-wider">
              {l.label}
            </a>
          ))}
        </div>
        ...
        <div className="md:hidden bg-foreground/95 backdrop-blur-md border-t border-border/20 pb-4">
          {links.map((l) => (
            <a
              ...
              className="block px-6 py-3 text-primary-foreground/80 hover:text-primary transition-colors font-body text-sm uppercase tracking-wider"
            >
              {l.label}
            </a>
          ))}
